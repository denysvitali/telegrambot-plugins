...
local function process_google_data(google, receiver, query)
  ...
  elseif google.responseStatus == 200 then
    ...
    local base = "http://memegen.vps.denv.it/"
    local furl = base .. "?imageURL=" ..path .. '&top_text=' .. top .. '&bottom_text=' .. down
...
return {
   description = "Generates meme based on an image (searched via google), an optional top text and an optional bottom text",
   usage = {
      "!genmeme <image name>-[toptext]-[bottomtext]"
   },
   patterns = {
      "^!genmeme (.+)-(.*)-(.*)$",
   },
